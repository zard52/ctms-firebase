...
class UpdateFlight extends Component {
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', width: '100%', margin: '20px 0px' }}>
            <ThemeProvider theme={customPostTheme}>
              <Button
                ...
                disabled={loading}
                onClick={() => this.updateSurvey(pk, orgAirport, destAirport, date)}
              >
  ...
  async updateSurvey(pk, origin, destination, date) {
    if (!this.state.success) {
      if (origin && destination && date) {
        ...
        await this.props.firebase.updateSurvey(pk, surveyRef)
  ...
  renderFlights(origin, destination, date) {
    const surveyRef = this.props.firebase.survey()
    const filteredFlights = surveyRef
